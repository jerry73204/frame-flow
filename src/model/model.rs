...
impl Generator {
    pub fn new<'a>(path: impl Borrow<nn::Path<'a>>) -> Self {
        ...
        let seq = nn::seq_t()
            ...
            .add(nn::batch_norm2d(path / "bn2", 512, Default::default()))
            .add_fn(|xs| xs.relu())
            .add(tr2d(path / "tr3", 512, 256, 1, 2))
            .add(nn::batch_norm2d(path / "bn3", 256, Default::default()))
            .add_fn(|xs| xs.relu())
            .add(tr2d(path / "tr4", 256, 128, 1, 2))
            .add(nn::batch_norm2d(path / "bn4", 128, Default::default()))
            .add_fn(|xs| xs.relu())
            .add(tr2d(path / "tr5", 128, 3, 1, 2))
            .add_fn(|xs| xs.tanh());
        Self { seq }
    }

    pub fn forward_t(&mut self, input: &Tensor, train: bool) -> Result<Tensor> {
        let output = input.apply_t(&self.seq, train);
        Ok(output)
    }
}
...
impl Discriminator {
    pub fn new<'a>(path: impl Borrow<nn::Path<'a>>) -> Self {
        let path = path.borrow();
        let seq = nn::seq_t()
            .add(conv2d(path / "conv1", 3, 128, 1, 2))
            .add_fn(leaky_relu)
            .add(conv2d(path / "conv2", 128, 256, 1, 2))
            .add(nn::batch_norm2d(path / "bn2", 256, Default::default()))
            .add_fn(leaky_relu)
            .add(conv2d(path / "conv3", 256, 512, 1, 2))
            .add(nn::batch_norm2d(path / "bn3", 512, Default::default()))
            .add_fn(leaky_relu)
            .add(conv2d(path / "conv4", 512, 1024, 1, 2))
            .add(nn::batch_norm2d(path / "bn4", 1024, Default::default()))
            .add_fn(leaky_relu)
            .add(conv2d(path / "conv5", 1024, 1, 0, 1));
        Self { seq }
    }
